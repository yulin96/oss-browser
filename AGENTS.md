...
- Keep `release-notes.md` as the user-facing notes for the next release. Update it in the same change whenever functionality, UI, behavior, compatibility, or a user-visible bug fix changes. It may remain empty when there is no user-visible change; do not add test-only, formatting, refactoring, or internal build details.
